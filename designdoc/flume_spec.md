...
- 外部依存は polars + structlog。それ以外は標準ライブラリで完結させる（graphlib, concurrent.futures, threading, dataclasses）。structlog の採用理由・使い方は `flume_logging_spec.md` §1 を参照。
...
├── pyproject.toml
├── src/moktan/
│   ├── __init__.py      # 公開API: Node, run, PipelineError, CycleError, RunRecorder, configure_logging の re-export
│   ├── node.py          # Node 定義
│   ├── graph.py         # DAG 収集・toposort・閉路検出
│   ├── runner.py        # 実行エンジン（stale判定・並列実行・メモリ解放）
│   ├── events.py        # 構造化イベント発行（_emit）・structlog 配線・configure_logging（flume_logging_spec.md）
│   └── recorder.py      # RunRecorder（Mermaid/Markdown 可視化、flume_logging_spec.md §7）
└── tests/
```
...
- `PipelineError(RuntimeError)`: ノード実行またはロードの失敗。`node: Node` 属性(最初に失敗したノード)と `__cause__` に元例外を持つ。`failed: list[Node]` にこの run で失敗した全ノードを完了処理順で持つ(`failed[0] is node`)。複数ノードが並列に失敗した場合は最初の 1 件を raise し、残りは `__notes__` に path を追記する（ExceptionGroup は使わない。呼び出し側の except が煩雑になるため）。`failed` は `flume_logging_spec.md` の `run_failed` イベントの `failed` フィールドと対応する。
...
- Pass 2: ロード対象（pass2 に含めてロードするノード集合）は「再計算ノードの依存として実際に必要なノード集合（再計算ノードの deps の閉包 ∩ 非再計算ノード）」に加えて **root を必ず含める**。root は他ノードの dep になり得ない（sink）ため上記の閉包だけでは fresh な root が漏れるので、明示的に足す。これにより `needs_compute[root]` が False（全ノード fresh）のケースも Pass 2 の通常経路が処理する ── 専用の早期リターンは持たない（rev3 §1.2: 失敗イベント発行の経路を一本化するため）。root 以外の fresh な祖先ノードで下流に再計算ノードが 1 つもないものは、従来どおりロード自体を省略する。
...
`moktan.events._emit` 経由の構造化イベントを発行する。print 禁止。詳細なイベント仕様・出力フォーマット・可視化(`RunRecorder`)は `flume_logging_spec.md` を参照。要約:

> `_emit` 経由の構造化イベント(flume_logging_spec.md §3 の表)を発行する。**正常に完走した run では、到達可能な全ノードが `node_computed` / `node_loaded` / `node_skipped` のうちちょうど 1 つの INFO イベントを発行する。** 失敗した run では、失敗ノードは `node_failed`(ERROR)をちょうど 1 つ発行し、cancelled ノードは `node_cancelled`(DEBUG)をちょうど 1 つ、未 submit のノードは何も発行しない。`run_started` と `run_finished` / `run_failed` は全 run で対になる。

コンソール向けレンダリングは後方互換のため `computed <path> (X.XXs)` / `loaded <path>` / `skipped <path>` の先頭 2 トークン(`split()[:2]` で verb/path が取れる)を維持する(flume_logging_spec.md §6.1)。
