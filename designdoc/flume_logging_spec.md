...
**既知の制約(受容済み)**: 2 番目のトークン(`<path>`)は `split()[:2]` 契約を保つため裸(無引用符)で出力する。そのため path にスペースが含まれる場合、この位置だけは `split()[:2]` が壊れる ── これは構造的な制約であり、スペースを含む path をエスケープする手段はない(エスケープすればトークン自体が変わり契約に反する)。改行だけは「1 イベント1行」の不変条件そのものを壊すため `\n`(バックスラッシュ+n)へ変換して防ぐ。同様に `deps` のようなリスト値の要素(path)も、要素ごとに常時クォートされる前提のため通常は `'...'` の repr 形式で問題ないが、要素に `"` や改行が含まれる場合のみ `json.dumps` 形式(二重引用符)にフォールバックする ── ここでもスペース単体は元の repr 形式のまま残る、という同じ受容済み制約が適用される。
