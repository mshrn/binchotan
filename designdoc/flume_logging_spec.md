...
|---|---|---|---|
| `run_started` | INFO | `run()` 冒頭、Pass 1 の前(main) | `root: str`, `force: bool`, `max_workers: int` |
...
| `node_computed` | INFO | `f` 実行 + atomic write 完了時(worker または main) | `duration_s: float`, `rows: int`, `columns: int`, `bytes: int`(書き込み後の `stat().st_size`) |
...
| `run_failed` | ERROR | `PipelineError` raise 直前(main) | `status: "failed"`, `duration_s: float`, `failed: list[str]`(node path 一覧) |

補足:

- `node_planned` の全ノード分の `deps` により、**イベントストリームだけから DAG が完全に再構成できる**(可視化の前提。1 イベントに全エッジを詰める案は大規模グラフでイベントが肥大するため却下し、ノードごとに分割)。
- fresh-root 早期リターン経路では `node_loaded`(root)+ `node_skipped`(他の全ノード)を発行し、`run_finished` まで通常どおり出す。
...
- `node_planned` の `decision` は Plan から導出: recompute → `"compute"`、pass2 ∩ 非 recompute → `"load"`、それ以外 → `"skip"`。**ただし root だけは `"skip"` にならない**: 全ノード fresh のとき root は pass2 外だが、fresh-root 早期リターンで必ずロードされるため `decision="load"` とする(`plan_computed` のカウントも同様に root を n_load に数える)。
...
### 12.6 ケース6: 全ノード fresh の再実行 — fresh-root 早期リターンと skipped

ケース1の直後に何も変更せず `run(joined, max_workers=1)` をもう一度実行する。全ノードが fresh なので再計算は発生せず、root のロード 1 回だけで返る(flume_spec §4 Pass 2 の省略ロード)。**日常運用で最も高頻度に出るログ形**であり、`node_skipped` と `decision="skip"` が登場する唯一の正常系でもある。
...
- `n_load=1` は root 自身。§4 の root 特例(fresh root は `decision="load"`)により、`node_planned` の decision と実際の INFO イベントが 1:1 で対応する(skip×3 → `skipped`×3、load×1 → `loaded`×1)。
