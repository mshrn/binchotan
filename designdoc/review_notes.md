...
- **指示書の粒度**(rev6 改訂の経緯): 指示書に選択肢(「(a)/(b) を判断して実装」)や裁量(「〜省略可」)を残すと、実装側の解釈余地がバグ再発の温床になる。以後の指示書は「全意思決定済み・シンボル名アンカー(行番号は実装中にずれる)・貼り付け可能なコード断片・受入テスト(xfail strict=True)同梱」の形式で書く。受入テストのプロトコルは tests/test_rev6_acceptance.py の docstring を参照(テスト本体は変更禁止、修正した項目のマーカーだけ外す、red 確認は --runxfail)。
