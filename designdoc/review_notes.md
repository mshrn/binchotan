...
- **受入テストファイルのライフサイクル**(rev7 §4.1 で新設): 受入テストファイル(tests/test_revN_acceptance.py)の寿命は、rev 完了(全マーカー解除+最終検証 green)をもって終わる。各テストを §参照 docstring ごとテーマ別ファイルへ移設し、rev ファイル自体は削除する。恒久リグレッションはテーマで引けるべきで、rev 番号のファイルを蓄積しない。移設は次 rev の指示書に必ず含める(rev7 §2.3 が初適用)。
