...
## 5.5 実装後追記(§4.2 に基づく規定外変更・逸脱の正規化 — rev7 完了レビューで確定)

rev7 実装のレビュー(3 ファインダー)で検出された逸脱と、その裁定・修正:

1. **§1.1 修正 4 の挿入位置**: 当初 `_emit` の説明の後ろに挿入され原因帰属が濁っていた。
   規定どおり「excepted):」直後へ移動して修正済み。
2. **§1.1 の grep 不変条件**: 規定の `grep -E "[^_a-z]str\(exc\)" src/moktan/` は
   コメント/docstring と graph.py の CycleError メッセージ構築(イベントフィールド外)に
   ヒットし 0 件が構造的に不可能だった。不変条件を
   `grep -nE "(^|[^_a-z])str\(exc\)" src/moktan/runner.py` = 0 件に精密化
   (review_notes.md も同時修正)。チェックリスト項目 2 の当初の「0 件」verification は
   この精密化前の範囲では実施されていなかったことを記録する。
3. **§1.2 の規定コメント**: 辞書 2 行の末尾コメント(ASCII エスケープ表記の維持指示)が
   貼付時に脱落していた。追補済み(バイト検査で生文字なしを再確認)。
4. **§2.3 の jsonl テスト**: 規定は「単体で移設」だったが、移設先に同一シナリオの
   rev5 §1.2 テストが既存だったため、新規重複を作らず**既存テストへアサーションを統合**
   (timestamp 正規表現・run_id・event 名すべて保持、ファインダーが union を検証済み)。
   本節をもって正規の裁定とする。
5. **§2.3 の warning 型名テスト**: test_events.py への移設は重複かつテーマ違いだった
   (broken-sink 分離は test_logging_examples.py がテーマ元)。単発テストを廃し、
   `_assert_broken_sink_isolated` に型名アサートを追加して全 10 パラメトライズケースで
   ピンする形に統合。
6. **§4.1 の review_notes 文面**: 貼付時に語尾が書き換わっていたため規定文面に復元。

## 6. 作業順序(チェックリスト)【完了】

各項目は**検証後にのみ** [x] を付けること(§4.2)。

1. [x] §1.2(U+2028 表記修正)+ 確認コマンド 2 種 green(バイト検査で確認)。
2. [x] §1.1 修正 1〜4 → `grep -E "[^_a-z]str\(exc\)" src/moktan/` 0 件 →
       受入テスト 2 本のマーカー解除 → PASSED 確認(worktree `ac063b4` で red 確認済み)。
3. [x] §2.1(scalar/deps パラメトライズ、20 ケース)→ 旧 \r クローン 2 本削除 →
       test_events.py 単独 green(58 passed)。
4. [x] §2.2(rev6 受入の exotic 16 ケース削除)→ §2.3(残り 5 テスト移設 +
       ファイル削除 + `MOKTAN_TIMESTAMP_RE` の conftest 集約 + review_notes 参照更新)。
       jsonl フォールバックテストは重複を避け既存の rev5 §1.2 テストへ統合。
5. [x] §3.1〜§3.3(spec 追記・コメント差し替え)。
6. [x] §4.1/§4.2(review_notes.md 追記)。
7. [x] rev7 受入テストの移設(§4.1 規約に従い test_runner.py へ)+
       `tests/test_rev7_acceptance.py` 削除。
8. [x] 最終検証: 指示書との逐語 diff(§4.2、`_safe_str`/grep 確認済み)→
       `uv run pytest`(全ファイル単独も 128 passed)+ `uv run ty check`(green)+
       PBT random seed 3 回(green)+ 全体連続 5 回(green)。
